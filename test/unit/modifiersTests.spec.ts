...
contract("SavingAccount.withdraw", async (accounts) => {
    ...
    context("onlyOwner", async () => {
        context("should fail", async function () {
            this.timeout(0);
            it("when someone other than the owner calls", async () => {
                await expectRevert(
                    tokenInfoRegistry.initialize(globalConfig.address, {
                        from: user1,
                    }),
        ...
        context("should succeed", async () => {
            it("when the owner calls the repective function", async function () {
                this.timeout(0);
