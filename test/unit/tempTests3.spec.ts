...
contract("SavingAccount.withdraw", async (accounts) => {
    ...
    context("Test", async () => {
        context("should succ", async () => {
            it("repay(): Case 1", async () => {
                const oneWBTC = eightPrecision;

                await erc20WBTC.transfer(user1, oneWBTC.mul(new BN(20)));
                await erc20WBTC.approve(savingAccount.address, oneWBTC.mul(new BN(20)), { from: user1 });
                await savingAccount.deposit(erc20WBTC.address, oneWBTC.mul(new BN(20)), { from: user1 });

                await savingAccount.borrow(erc20WBTC.address, oneWBTC.div(new BN(2)), { from: user1 });
