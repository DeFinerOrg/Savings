                ...
                async () => {
                    ...
                    context("Should succeeed", async () => {
                        it("Deposits WBTC, borrows a small amount of TUSD ", async function () {
                            ...
                            const savingsCompoundWBTCBeforeDeposit = new BN(await cTokenWBTC.balanceOfUnderlying.call(savingAccount.address))
                            ...
                            const savingsCompoundWBTCAfterDeposit = new BN(await cTokenWBTC.balanceOfUnderlying.call(savingAccount.address))
                            ...
                            const savingsCompoundWBTCAfterBorrow = new BN(await cTokenWBTC.balanceOfUnderlying.call(savingAccount.address))

                            const user1TUSDAfter = await erc20TUSD.balanceOf(user1);

                            expect(BN(user1TUSDAfter).sub(BN(user1TUSDBefore))).to.be.bignumber.equals(
                                borrow
                            );

                            expect(savingsCompoundWBTCAfterDeposit.sub(savingsCompoundWBTCBeforeDeposit))
                                .to.be.bignumber.
                                equals(eightPrecision.mul(new BN(1)).mul(new BN(85)).div(new BN(100)));

                            expect(savingsCompoundWBTCAfterBorrow.sub(savingsCompoundWBTCAfterDeposit))
                                .to.be.bignumber.
                                equals(new BN(0));
                        });
                        ...
                        it("Deposits TUSD, borrows the same amount of borrowing power", async function () {
                            ...
                            expect(BN(user1WBTCAfter).sub(BN(user1WBTCBefore))).to.be.bignumber.equals(
                                borrow
                            );

                            expect(savingsCompoundWBTCAfterDeposit.sub(savingsCompoundWBTCBeforeDeposit))
                                .to.be.bignumber.
                                equals(eightPrecision.mul(new BN(100)).mul(new BN(85)).div(new BN(100)));
        ...
        context("Call multiple times", async () => {
            context("Should succeed", async () => {
                it("Uses 18 decimals, TUSD", async function () {
                    ...
                    const savingsCompoundDAIBeforeDeposit = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address));
                    ...
                    const savingsCompoundDAIAfterDeposit = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address));
                    ...
                    const savingsCompoundDAIAfterFirstBorrow = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address));

                    let accTUSDAfterFirst = await erc20TUSD.balanceOf(user1);

                    await savingAccount.borrow(addressTUSD, borrow, { from: user1 });

                    const savingsCompoundDAIAfterSecondBorrow = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address));
                    ...
                    expect(savingsCompoundDAIAfterDeposit.sub(savingsCompoundDAIBeforeDeposit))
                        .to.be.bignumber.
                        equals(eighteenPrecision.mul(new BN(100)).mul(new BN(85)).div(new BN(100)));

                    expect(savingsCompoundDAIAfterSecondBorrow.sub(savingsCompoundDAIAfterFirstBorrow))
                        .to.be.bignumber.
                        equals(new BN(0));

                    expect(savingsCompoundDAIAfterFirstBorrow.sub(savingsCompoundDAIAfterSecondBorrow))
                        .to.be.bignumber.
                        equals(new BN(0));
                ...
                it("Uses 6 decimals, USDC", async function () {
                    ...
                    const savingsCompoundDAIBeforeDeposit = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address))
                    const savingsCompoundUSDCBeforeDeposit = new BN(await cTokenUSDC.balanceOfUnderlying.call(savingAccount.address))
                    ...
                    const savingsCompoundDAIAfterDeposit = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address))
                    const savingsCompoundUSDCAfterDeposit = new BN(await cTokenUSDC.balanceOfUnderlying.call(savingAccount.address))
                    /*
                     * Step 2
                    ...
                    const savingsCompoundDAIAfterFirstBorrow = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address));
                    const savingsCompoundUSDCAfterFirstBorrow = new BN(await cTokenUSDC.balanceOfUnderlying.call(savingAccount.address));
                    ...
                    const savingsCompoundDAIAfterSecondBorrow = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address));
                    const savingsCompoundUSDCAfterSecondBorrow = new BN(await cTokenUSDC.balanceOfUnderlying.call(savingAccount.address));
                    ...
                    expect(savingsCompoundUSDCAfterDeposit.sub(savingsCompoundUSDCBeforeDeposit))
                        .to.be.bignumber.
                        equals(sixPrecision.mul(new BN(100)).mul(new BN(85)).div(new BN(100)));

                    expect(savingsCompoundUSDCAfterDeposit.sub(savingsCompoundUSDCAfterFirstBorrow))
                        .to.be.bignumber.
                        equals(borrow);

                    expect(savingsCompoundUSDCAfterFirstBorrow.sub(savingsCompoundUSDCAfterSecondBorrow))
                        .to.be.bignumber.
                        equals(borrow);
                ...
                it("Uses 8 decimals, WBTC", async function () {
                    ...
                    const savingsCompoundDAIAfterFirstBorrow = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address))
                    const savingsCompoundWBTCAfterFirstBorrow = new BN(await cTokenWBTC.balanceOfUnderlying.call(savingAccount.address))

                    let accWBTCAfterFirst = await erc20WBTC.balanceOf(user1);
                    await savingAccount.borrow(addressWBTC, borrow, { from: user1 });

                    const savingsCompoundDAIAfterSecondBorrow = new BN(await cTokenDAI.balanceOfUnderlying.call(savingAccount.address))
                    const savingsCompoundWBTCAfterSecondBorrow = new BN(await cTokenWBTC.balanceOfUnderlying.call(savingAccount.address))
                    ...
                    expect(savingsCompoundDAIAfterDeposit.sub(savingsCompoundDAIBeforeDeposit))
                        .to.be.bignumber.
                        equals(eighteenPrecision.mul(new BN(1000)).mul(new BN(85)).div(new BN(100)));
                    ...
                    expect(savingsCompoundWBTCAfterDeposit.sub(savingsCompoundWBTCBeforeDeposit))
                        .to.be.bignumber.
                        equals(eightPrecision.mul(new BN(1000)).mul(new BN(85)).div(new BN(100)));

                    expect(savingsCompoundWBTCAfterDeposit.sub(savingsCompoundWBTCAfterFirstBorrow))
                        .to.be.bignumber.
                        equals(new BN(0));

                    expect(savingsCompoundWBTCAfterFirstBorrow.sub(savingsCompoundWBTCAfterSecondBorrow))
                        .to.be.bignumber.
                        equals(new BN(0));
                });
            });
