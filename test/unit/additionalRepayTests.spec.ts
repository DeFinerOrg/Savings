        ...
        context(
            ...
            async () => {
                it("Deposit DAI, borrows USDC and wants to withdraw", async function () {
                    ...
                    const CTokenLeftAfter = new BN(
                        await cTokenUSDC.balanceOfUnderlying.call(savingAccount.address)
                    );

                    const TokenLeftAfter = new BN(await erc20USDC.balanceOf(savingAccount.address));
                    const CTokenExpected = new BN(numOfUSDC).mul(new BN(85)).div(new BN(100));
                    const TokenExpected = new BN(numOfUSDC).mul(new BN(15)).div(new BN(100));

                    expect(TokenLeft).to.be.bignumber.equal(ZERO);
                    expect(CTokenLeftAfter).to.be.bignumber.equal(CTokenExpected);
                    expect(TokenLeftAfter).to.be.bignumber.equal(TokenExpected);
        ...
        context("Checking saving account's value after repayment", async () => {
            context("Should succeed.", async () => {
                it("Repay all the outstandings DAI token", async function () {
                    ...
                    const realCTokenBalance = await cTokenDAI.balanceOfUnderlying.call(savingAccount.address);
                    ...
                    expect(savingAccountCDAITokenAfterDeposit.sub(savingAccountCDAITokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(numOfDAI).mul(new BN(50)).div(new BN(100)));
                ...
                it("Repay half the outstandings DAI token", async function () {
                    ...
                    expect(savingAccountCDAITokenAfterRepay.sub(savingAccountCDAITokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(numOfDAI).mul(new BN(25)).div(new BN(100)));
        ...
        context("Repay partially several times.", async () => {
            context("Use DAI, should succeed", async () => {
                it("Repay twice, every time repay 0.25 * 10^18 DAI tokens", async function () {
                    ...
                    const savingAccountCDAITokenAfterFirstRepay = BN(await cTokenDAI.balanceOfUnderlying.call(
                        savingAccount.address
                    ));
                    ...
                    const savingAccountCDAITokenAfterSecondRepay = BN(await cTokenDAI.balanceOfUnderlying.call(
                        savingAccount.address
                    ));
                    ...
                    expect(savingAccountCDAITokenAfterDeposit.sub(savingAccountCDAITokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(numOfDAI).mul(new BN(50)).div(new BN(100)));
                    expect(savingAccountCDAITokenAfterFirstRepay.sub(savingAccountCDAITokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(numOfDAI).mul(new BN(25)).div(new BN(100)));
                    expect(savingAccountCDAITokenAfterSecondRepay.sub(savingAccountCDAITokenAfterFirstRepay))
                        .to.be.bignumber
                        .equal(new BN(numOfDAI).mul(new BN(25)).div(new BN(100)));
                });
            });
            context("Use USDC, should succeed", async () => {
                it("Repay twice, every time repay 0.25 * 10^6 USDC tokens", async function () {
                    ...
                    const savingAccountCUSDCTokenAfterFirstRepay = BN(await cTokenUSDC.balanceOfUnderlying.call(
                        savingAccount.address
                    ));
                    const userBalanceAfterFirstRepay = await accountsContract.getBorrowBalanceCurrent(
                        addressUSDC,
                    ...
                    await savingAccount.repay(addressUSDC, quaterOfUSDC, { from: user1 });
                    const savingAccountCUSDCTokenAfterSecondRepay = BN(await cTokenUSDC.balanceOfUnderlying.call(
                        savingAccount.address
                    ));
                    const userBalanceAfterSecondRepay = await accountsContract.getBorrowBalanceCurrent(
                        addressUSDC,
                    ...
                    expect(savingAccountCUSDCTokenAfterDeposit.sub(savingAccountCUSDCTokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(numOfUSDC).mul(new BN(50)).div(new BN(100)));
                    expect(savingAccountCUSDCTokenAfterFirstRepay.sub(savingAccountCUSDCTokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(numOfUSDC).mul(new BN(25)).div(new BN(100)));
                    expect(savingAccountCUSDCTokenAfterSecondRepay.sub(savingAccountCUSDCTokenAfterFirstRepay))
                        .to.be.bignumber
                        .equal(new BN(numOfUSDC).mul(new BN(25)).div(new BN(100)));
                });
            });
            ...
            context("should succeed", async () => {
                ...
                it("When the repayment DAI Amount is less than the loan amount.", async function () {
                    ...
                    expect(savingAccountCWBTCTokenAfterDeposit.sub(savingAccountCWBTCTokenBeforeDeposit))
                        .to.be.bignumber
                        .equal(new BN(numOfWBTC).mul(new BN(85)).div(new BN(100)));
                    expect(savingAccountCDAITokenAfterDeposit.sub(savingAccountCDAITokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(0));
                    expect(savingAccountCDAITokenAfterRepay.sub(savingAccountCDAITokenAfterBorrow))
                ...
                it("When the repayment WBTC Amount is less than the loan amount.", async function () {
                    ...
                    const savingAccountCWBTCTokenAfterBorrow = BN(await cTokenWBTC.balanceOfUnderlying.call(
                        savingAccount.address
                    ));
                    ...
                    const savingAccountCWBTCTokenAfterRepay = BN(await cTokenWBTC.balanceOfUnderlying.call(
                        savingAccount.address
                    ));
                    // 4. Verify the repay amount.
                    ...
                    expect(savingAccountCWBTCTokenAfterDeposit.sub(savingAccountCWBTCTokenBeforeDeposit))
                        .to.be.bignumber
                        .equal(new BN(numOfWBTC).mul(new BN(85)).div(new BN(100)));
                    expect(savingAccountCWBTCTokenAfterDeposit.sub(savingAccountCWBTCTokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(0));
                    expect(savingAccountCWBTCTokenAfterRepay.sub(savingAccountCWBTCTokenAfterBorrow))
                        .to.be.bignumber
                        .equal(new BN(0));
                });
