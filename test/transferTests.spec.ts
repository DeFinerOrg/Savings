        ...
        context("with Token", async () => {
            context("should fail", async () => {
                it("Not enough balance for transfer", async () => {
                    ...
                    let user1BalanceAfterDeposit = await savingAccount.getDepositBalance(addressDAI, user1);
                    let user2BalanceAfterDeposit = await savingAccount.getDepositBalance(addressDAI, user2);
                    expect(new BN(user1BalanceAfterDeposit)).to.be.bignumber.equal(numOfToken);
                    expect(new BN(user2BalanceAfterDeposit)).to.be.bignumber.equal(numOfToken);
            ...
            context("should succeed", async () => {
                it("Transfer small amount balance", async () => {
                    ...
                    let user1BalanceAfterTransfer = await savingAccount.getDepositBalance(addressDAI, user1);
                    let user2BalanceAfterTransfer = await savingAccount.getDepositBalance(addressDAI, user2);
        ...
        context("with ETH", async () => {
            ...
            context("should succeed", async () => {
                it("Transfer small amount balance", async () => {
                    ...
                    // Verify balances of user1 & user2 after deposit
                    let user1BalanceAfterDeposit = await savingAccount.getDepositBalance(ETH_ADDRESS, user1);
                    let user2BalanceAfterDeposit = await savingAccount.getDepositBalance(ETH_ADDRESS, user2);
                    expect(new BN(user1BalanceAfterDeposit)).to.be.bignumber.equal(
                        depositAmount
                    );
                    expect(new BN(user2BalanceAfterDeposit)).to.be.bignumber.equal(
                        depositAmount
                    );
                    ...
                    // Verify balances of user1 & user2 after transfer
                    let user1BalanceAfterTransfer = await savingAccount.getDepositBalance(ETH_ADDRESS, user1);
                    let user2BalanceAfterTransfer = await savingAccount.getDepositBalance(ETH_ADDRESS, user2);
                    expect(new BN(user1BalanceAfterTransfer)).to.be.bignumber.equal(
