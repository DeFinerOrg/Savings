        ...
        context("with Token", async () => {
            context("should succeed", async () => {
                ...
                it("FinMiningTest1: Two users deposit in the same block with one borrow", async function () {
                    ...
                    expect(BN(totalBorrowInterest).add(totalCompoundInterest)).to.be.bignumber.equal(new BN(1272895842093200));
                    ...
                    expect(BN(balFIN)).to.be.bignumber.equal(new BN("50000000239482582980714"));
                });
                ...
                it("FinMiningTest2: Two user deposit in different blocks with one borrow", async function () {
                    ...
                    // totalBorrowInterest + totalCompundInterest = totalDepositInterest
                    expect(BN(totalBorrowInterest).add(totalCompoundInterest)).to.be.bignumber.equal(new BN(1272895875061756));
                    ...
                    await savingAccount.claim({ from: user1 });
                    const balFIN = await erc20FIN.balanceOf(user1);
                    expect(BN(balFIN)).to.be.bignumber.equal(new BN("150000001088281586483266"));
                    const balFINUser1Diff = BN(balFIN).sub(BN(balFINUser1));
                ...
                it("FinMiningTest3: Three user deposit in different blocks with one borrow", async function () {
                    ...
                    expect(BN(totalCompoundInterest)).to.be.bignumber.equal(new BN(19168623092)); // 19168622492
                    // totalBorrowInterest + totalCompundInterest = totalDepositInterest
                    expect(BN(totalBorrowInterest).add(totalCompoundInterest)).to.be.bignumber.equal(new BN(2422355194474031));
                    ...
                    await savingAccount.claim({ from: user1 });
                    const balFIN = await erc20FIN.balanceOf(user1);
                    expect(BN(balFIN)).to.be.bignumber.equal(new BN("183340403398805305814409"));
                    const balFINUser1Diff = BN(balFIN).sub(BN(balFINUser1));
                ...
                it("FinMiningTest4: Two user deposit in different blocks with two borrows", async function () {
                    ...
                    expect(BN(totalCompoundInterest)).to.be.bignumber.equal(new BN(7988042402));
                    // totalBorrowInterest + totalCompundInterest = totalDepositInterest
                    expect(BN(totalBorrowInterest).add(totalCompoundInterest)).to.be.bignumber.equal(new BN(2534577161794206));
                    ...
                    await savingAccount.claim({ from: user1 });
                    const balFIN = await erc20FIN.balanceOf(user1);
                    expect(BN(balFIN)).to.be.bignumber.equal(new BN("278632048349200181328203"));
                    const balFINUser1Diff = BN(balFIN).sub(BN(balFINUser1));
                ...
                it("FinMiningTest5: Three user deposit in different blocks with two borrows", async function () {
                    ...
                    expect(BN(totalCompoundInterest)).to.be.bignumber.equal(new BN(15973834614));
                    // totalBorrowInterest + totalCompundInterest = totalDepositInterest
                    expect(BN(totalBorrowInterest).add(totalCompoundInterest)).to.be.bignumber.equal(new BN(4285807764205468));
                    ...
                    await savingAccount.claim({ from: user1 });
                    const balFIN = await erc20FIN.balanceOf(user1);
                    expect(BN(balFIN)).to.be.bignumber.equal(new BN("340607524160808126200952")); // 7.85729643603791*0.2 + 1.8333341356503024
                    const balFINUser1Diff = BN(balFIN).sub(BN(balFINUser1));
